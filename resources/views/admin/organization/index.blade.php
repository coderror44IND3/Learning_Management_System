...
<div class="page-wrapper">
    ...
    <div class="content container-fluid">
        ...
        <div class="row">
            ...
            @foreach($organization as $organ_class)
            <div class="col-md-6 col-xl-4 col-sm-12 d-flex">
                <div class="blog grid-blog flex-fill">
                    ...
                    <div class="blog-image">
                        <a href="{{ route('organization.index') }}"><img class="img-fluid" src="{{ asset('admin/assets/img/category/blog-6.jpg') }}" alt="Post Image"></a>
                    </div>
                    ...
                    <div class="blog-content">
                            ...
                            <li>
                                <div class="post-author">
                                    <a href="{{ route('money.index') }}">
                                        ...
                                        <span>
                                            <span class="post-title"> Classroom {{ $organ_class->class_students }} </span>
                                            <span class="post-title mt-2"><i class="far fa-calendar-alt"> {{ $organ_class->date }} </i> &nbsp; <i class="far fa-clock mt-2"> {{ $money->clock }} </i> </span>
                                        </span>
                                    </a>
                                </div>
                            </li>
                        ...
                        @php
                        $status_money = $organ_class->status_students;
                        $btn_color = '';
                        ...
                        @endphp
                        <h3 class="blog-title"><a> {{ $organ_class->name }} </a> <span class="label-style badge text-center text-white {{ $btn_color }} rounded-3 fw-semibold"> {{ $status_money }} </span> </h3>
                    ...
                    <div class="row">
                        <div class="edit-options">
                            <div class="edit-delete-btn">
                                <form method="POST" action="{{ route('organization.destroy', $organ_class->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    @if(Auth::user()->role == 'Admin' || Auth::user()->role == 'Treasurer')
                                    <a href="{{ route('organization.edit', $organ_class->id) }}" style="height: 29px;" class="btn btn-sm bg-warning text-white"><i class="feather-edit me-1"></i> Edit</a>
